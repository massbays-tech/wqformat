test_that("format_sites works", {
  ...
  # Test conversions
  expect_equal(
    format_sites(dat_me_focb, 'ME_FOCB', 'MassWateR', show_messages = FALSE),
    dat_masswater
  )
  expect_equal(
    format_sites(dat_masswater, 'MassWateR', 'WQdashboard', show_messages = FALSE),
    dat_wqdashboard
  )

  # Test conversions - extra columns retained
  Town <- c('Providence', 'Boston', 'Portland')
  dat_extracol <- cbind(dat_masswater, Town)

  expect_equal(
    suppressWarnings(
      format_sites(
        dat_me_focb, 'ME_FOCB', 'MassWateR', drop_extra_col = FALSE,
        show_messages = FALSE
      )
    ),
    dat_extracol
  ...
  expect_warning(
    format_sites(
      dat_wqdashboard, 'WQdashboard', 'MassWateR', drop_extra_col = FALSE,
      show_messages = FALSE
    ),
